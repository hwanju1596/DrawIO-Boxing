...
def highlight_text(input_file, output_file, search_texts, padding=5): # padding added
    ...
    print(f"Found {len(unique_found_cells)} instance(s) of text matching prefixes. Highlighting them with padding {padding}...")
    for i, cell in enumerate(unique_found_cells):
        add_highlight_for_cell(cell.find("mxGeometry"), graph_model_root, tmpl_layer_id, f"text-highlight-{i}-{cell.get('id')}", padding) # padding passed

    tree.write(output_file, encoding='utf-8', xml_declaration=True)
    print(f"Created highlighted file: {output_file}")


def highlight_circles(input_file, output_file, ref_prefix="AR_G_2_", tolerance=0.5, padding=5): # padding added
    ...
    for prefix, _ in shape_prefixes.items():
        ...
        if current_sig == ref_sig:
            ...
            add_highlight_for_bbox(bbox_geom, graph_model_root, tmpl_layer_id, f"circle-highlight-{prefix}", padding) # padding passed

    if found_circles_count == 0:
        print("No matching circle shapes found to highlight.")
        return

    print(f"Found and highlighted {found_circles_count} circle shape(s) with padding {padding}.")
    tree.write(output_file, encoding='utf-8', xml_declaration=True)
    print(f"Created highlighted file: {output_file}")


def add_highlight_for_bbox(geom_attrs, graph_root, layer_id, base_id, padding=5): # padding added as parameter
...
def add_highlight_for_cell(geom_element, graph_root, layer_id, base_id, padding=5): # padding added
    """Wrapper to use the same highlighter for cells with mxGeometry."""
    if geom_element is None:
        return
    add_highlight_for_bbox(geom_element.attrib, graph_root, layer_id, base_id, padding)
...
if __name__ == '__main__':
    ...
    overall_padding = 10 # New default padding

    # Check for global padding argument first if it appears before mode
    temp_args = []
    while args:
        arg = args.pop(0)
        if arg == '--padding':
            if not args:
                ...
                sys.exit(1)
            try:
                overall_padding = int(args.pop(0))
            except ValueError:
    ...
    if mode_arg == '--text':
        ...
        highlight_text(input_f, output_f, search_texts, padding=overall_padding)
        ...
        if "text_prefixes" in config and isinstance(config["text_prefixes"], list):
            highlight_text(input_f, output_f, config["text_prefixes"], padding=overall_padding)
        ...
        highlight_circles(input_f, output_f, ref_prefix=ref_prefix, tolerance=tolerance, padding=overall_padding)
    else:
